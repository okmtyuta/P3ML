...
from src.main.experiments.abstract.transfer_predict_onehot_ccs_1 import transfer_predict_onehot_ccs_1
from src.modules.helper.helper import Helper
from src.modules.protein.protein_list import ProteinList
from src.modules.slack_service import SlackService

code = Path(__file__).stem
proteins = ProteinList.from_hdf5(Helper.ROOT / "source" / "meier" / "onehot" / "data.h5").proteins
...
def main() -> None:
    ...
    result = transfer_predict_onehot_ccs_1(
        code=code,
        from_code="EXP9-1",
        from_version="version_0",
        input_props=['charge', 'mass', 'length'],
        output_props=["ccs"],
        proteins=proteins,
    )
